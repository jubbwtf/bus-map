...
map.getView().fit([7673905.432789, 8619119.780634, 7690703.342812, 8630199.544959], map.getSize());
...
function getDistanceKm(lonLat1, lonLat2) {
    ...
    var a = Math.sin(dLat/2) * Math.sin(dLat/2) +
            Math.cos(lonLat1[1] * Math.PI / 180) * Math.cos(lonLat2[1] * Math.PI / 180) *
    ...
    distanceLineLayer = new ol.layer.Vector({
        source: new ol.source.Vector({
            features: [new ol.Feature({
                geometry: new ol.geom.LineString([userProj, TARGET_COORD])
            })]
        }),
        style: new ol.style.Style({
            stroke: new ol.style.Stroke({
...
// --- ИСПРАВЛЕНИЕ ИСЧЕЗНОВЕНИЯ ИНТЕРФЕЙСА НА МОБИЛЬНЫХ ---
// Переносим элементы в кастомные контейнеры ТОЛЬКО на десктопах.
// На телефонах (isSmallScreen) оставляем стандартное поведение, чтобы ничего не пропадало при скролле.
...
if (!isSmallScreen) {
    ...
    // Если это мобилка, скрываем пустые контейнеры, чтобы они не перехватывали тапы по карте
    if(topLeftContainerDiv) topLeftContainerDiv.style.display = 'none';
    if(bottomLeftContainerDiv) bottomLeftContainerDiv.style.display = 'none';
    if(bottomRightContainerDiv) bottomRightContainerDiv.style.display = 'none';
}

// --- ФИКС ПОП-АПА ПРИ СКРОЛЛЕ ДЛЯ МОБИЛОК ---
// Перерисовываем и удерживаем поп-ап при перемещении карты
if (isSmallScreen) {
    map.on('moveend', function() {
        if (container.style.display === 'block' && popupCoord) {
            overlayPopup.setPosition(popupCoord);
        }
    });
}